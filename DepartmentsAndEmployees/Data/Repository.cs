    ...
    public class Repository
    {
        ...
        public SqlConnection Connection
        {
            get
            {
                ...
                string _connectionString = "Server=DESKTOP-6QUVPEQ\\SQLEXPRESS;database=DepartmentsAndEmployees;Integrated Security=True;Connect Timeout=30;Encrypt=False;TrustServerCertificate=False;ApplicationIntent=ReadWrite;MultiSubnetFailover=False";
        ...
        public Employee GetEmployeeById(int id)
        {
            using (SqlConnection conn = Connection)
            {
                ...
                using (SqlCommand cmd = conn.CreateCommand())
                {
                    {
                        // String interpolation lets us inject the id passed into this method.
                        cmd.CommandText = $"FirstName, LastName, DepartmentId FROM Employee WHERE Id = {id}";
                        SqlDataReader reader = cmd.ExecuteReader();
                        List<Employee> employees = new List<Employee>();
                        if (reader.Read())
                        {
